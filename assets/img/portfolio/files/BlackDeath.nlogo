...
to setup-constants
  set lifespan 30 * 52      ;; 30 times 52 weeks = 30 years = 1560 weeks old
  set carrying-capacity 300
  set chance-reproduce 1
  set immunity-duration 52
  set ratlife 1 * 52 ;; 1 year old
end
...
;;
to setup-rats
  create-turtles number-rats
    [ setxy random-xcor random-ycor
      set age random ratlife
      set sick-time 0
      set remaining-immunity 0
      set color brown
...
to get-older-rat ;; rat procedure
  ;; rats die of old age once their age exceeds the
  ;; lifespan (set at 50 years in this model).
  set age age + 1
  if age > ratlife [ die ]
end
...
;; If a turtle is sick, it infects other turtles on the same patch.
;; Immune turtles don't get sick.
to infect-rats ;; turtle procedure
  ask other turtles-here with [ not sick? and not immune? ]
    [ if random-float 80 < infectiousness
      [ get-sick ] ]
end

to get-healthy-rats ;; turtle procedure
  set sick? true
  set remaining-immunity 20
  set sick-time 0
end
...
to update-global-variables
  if count turtles > 0
    [ set %infected (count turtles with [ sick? ] / count turtles) * 100
      ...
      set %rats (count turtles with [rats?]/ count turtles) * 100 ]
end

to update-display
  ask turtles
    [ if rats? != true [ set shape turtle-shape ]
      set color ifelse-value sick? [ red ] [ ifelse-value immune? [ grey ] [ green ] ] ]
end

to update-dis2.0
  ask turtles
  [ if rats? [ set shape rats-shape]]
end
...
"Rats" 1.0 0 -955883 true "" "plot count turtles with [rats?]"
